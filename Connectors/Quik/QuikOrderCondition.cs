...
namespace StockSharp.Quik
{
	...
	public class QuikOrderCondition : OrderCondition
	{
		...
		///</summary>
		/// <remarks>
		/// Значение <see langword="null"/> означает, что условие стоп-заявки ещё не сработало - стоп-заявка активна.
		/// </remarks>
		[DataMember]
		[CategoryLoc(LocalizedStrings.Str225Key)]
		...
		/// <summary>
		/// Время проверки условий заявки только в течение заданного периода времени (если значение <see langword="null"/>, то не проверять).
		...
		/// Если <see langword="false"/> (или <see langword="null"/>), то заявка «по исполнению» активируется только при полном исполнении заявки-условия <see cref="ConditionOrderId"/>. 
		/// </summary>
		[DataMember]
		[Nullable]
		[CategoryLoc(LocalizedStrings.Str225Key)]
		...
		/// принимается исполненный объем заявки-условия <see cref="ConditionOrderId"/>. Если <see langword="false"/> (или <see langword="null"/>), то объем заявки указывается явно в свойство <see cref="Order.Volume"/>.
		/// </summary>
		[DataMember]
		[Nullable]
		[CategoryLoc(LocalizedStrings.Str225Key)]
