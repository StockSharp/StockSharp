...
public class ExcelExporter(IExcelWorkerProvider provider, DataType dataType, Stream stream, Action breaked) : BaseExporter(dataType)
{
	...
	/// <inheritdoc />
	protected override async Task<(int, DateTime?)> ExportOrderLogAsync(IAsyncEnumerable<ExecutionMessage> messages, CancellationToken cancellationToken)
	{
	...
	/// <inheritdoc />
	protected override async Task<(int, DateTime?)> ExportTicksAsync(IAsyncEnumerable<ExecutionMessage> messages, CancellationToken cancellationToken)
	{
		using var worker = CreateWorker();
	...
	/// <inheritdoc />
	protected override async Task<(int, DateTime?)> ExportTransactionsAsync(IAsyncEnumerable<ExecutionMessage> messages, CancellationToken cancellationToken)
	{
		using var worker = CreateWorker();
	...
	/// <inheritdoc />
	protected override async Task<(int, DateTime?)> Export(IAsyncEnumerable<QuoteChangeMessage> messages, CancellationToken cancellationToken)
	{
		using var worker = CreateWorker();
	...
	/// <inheritdoc />
	protected override async Task<(int, DateTime?)> Export(IAsyncEnumerable<Level1ChangeMessage> messages, CancellationToken cancellationToken)
	{
		using var worker = CreateWorker();

		var columns = new Dictionary<Level1Fields, int>();

		worker.SetCell(0, 0, LocalizedStrings.Time).SetStyle(0, "yyyy-MM-dd HH:mm:ss.fff");
	...
	/// <inheritdoc />
	protected override async Task<(int, DateTime?)> Export(IAsyncEnumerable<PositionChangeMessage> messages, CancellationToken cancellationToken)
	{
		using var worker = CreateWorker();
	...
	/// <inheritdoc />
	protected override async Task<(int, DateTime?)> Export(IAsyncEnumerable<IndicatorValue> values, CancellationToken cancellationToken)
	{
		using var worker = CreateWorker();
	...
	/// <inheritdoc />
	protected override async Task<(int, DateTime?)> Export(IAsyncEnumerable<CandleMessage> messages, CancellationToken cancellationToken)
	{
		using var worker = CreateWorker();
		...
		await foreach (var candle in messages.WithCancellation(cancellationToken))
		{
			worker
	...
	/// <inheritdoc />
	protected override async Task<(int, DateTime?)> Export(IAsyncEnumerable<NewsMessage> messages, CancellationToken cancellationToken)
	{
		using var worker = CreateWorker();
		...
		await foreach (var n in messages.WithCancellation(cancellationToken))
		{
			worker
				.SetCell(0, row, n.Id)
	...
	/// <inheritdoc />
	protected override async Task<(int, DateTime?)> Export(IAsyncEnumerable<SecurityMessage> messages, CancellationToken cancellationToken)
	{
		using var worker = CreateWorker();
		...
		await foreach (var security in messages.WithCancellation(cancellationToken))
		{
			colIndex = 0;
	...
	/// <inheritdoc />
	protected override async Task<(int, DateTime?)> Export(IAsyncEnumerable<BoardStateMessage> messages, CancellationToken cancellationToken)
	{
		using var worker = CreateWorker();
	...
	/// <inheritdoc />
	protected override async Task<(int, DateTime?)> Export(IAsyncEnumerable<BoardMessage> messages, CancellationToken cancellationToken)
	{
		using var worker = CreateWorker();
	...
	private IExcelWorker CreateWorker()
	{
		var worker = _provider.CreateNew(stream);
