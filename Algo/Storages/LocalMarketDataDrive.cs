...
public class LocalMarketDataDrive : BaseMarketDataDrive
{
	private class LocalMarketDataStorageDrive : IMarketDataStorageDrive
	{
		...
		private IEnumerable<DateTime> LoadDates()
		{
			try
			{
				var bytes = _fileSystem.ReadAllBytes(_datesPath);
		...
		private IEnumerable<DateTime> LoadDatesObsoleteBin()
		{
			try
			{
				var bytes = _fileSystem.ReadAllBytes(_datesPathObsoleteBin);
		...
		private void SaveDates(DateTime[] dates)
		{
			try
			{
				...
				using (_cacheSync.EnterScope())
				{
					_fileSystem.WriteAllBytes(_datesPath, stream.ToArray());
	...
	private bool TryGetIndex(out Index index)
	{
		...
		using (_indexLock.EnterScope())
		{
			...
			{
				index = [];
				var indexBytes = _fileSystem.ReadAllBytes(IndexFullPath);
	...
	public override IAsyncEnumerable<SecurityMessage> LookupSecuritiesAsync(SecurityLookupMessage criteria, ISecurityProvider securityProvider)
	{
		...
		async IAsyncEnumerable<SecurityMessage> Impl([EnumeratorCancellation] CancellationToken cancellationToken = default)
		{
			...
			foreach (var letterDir in _fileSystem.GetDirectories(Path))
			{
				...
				securityPaths.AddRange(_fileSystem.GetDirectories(letterDir));
			}
			...
			foreach (var securityPath in securityPaths)
			{
				...
				if (IOPath.GetExtension(firstDataFile).EqualsIgnoreCase(".bin"))
				{
					try
					{
						var fileBytes = await _fileSystem.ReadAllBytesAsync(firstDataFile, cancellationToken);
