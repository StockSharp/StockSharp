...
public class BufferMessageAdapter(IMessageAdapter innerAdapter, StorageCoreSettings settings, IStorageBuffer buffer, ISnapshotRegistry snapshotRegistry) : MessageAdapterWrapper(innerAdapter)
{
	...
	protected override async ValueTask OnSendInMessageAsync(Message message, CancellationToken cancellationToken)
	{
		switch (message.Type)
		{
			...
			case MessageTypes.MarketData:
			{
				...
				// A snapshot is data for this subscription, and data for a subscription that has not
				// been passed on yet belongs to nobody, so it follows rather than precedes it.
	...
	private async ValueTask ProcessOrderStatusAsync(OrderStatusMessage message, CancellationToken cancellationToken)
	{
		...
		// The subscription is acknowledged before what belongs to it is sent, the same way the
		// connection would acknowledge it.
		if (covered)
