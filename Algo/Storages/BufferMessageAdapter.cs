namespace StockSharp.Algo.Storages
{
	...
	public class BufferMessageAdapter : MessageAdapterWrapper
	{
		...
		private static readonly TimeSpan AutoFlushInterval = TimeSpan.FromSeconds(10);
		...
		private bool _storageTaskStarted;
		private DateTime _lastFlushTime = DateTime.UtcNow;
		private readonly SyncObject _flushSyncObject = new SyncObject();
		private TimeSpan TimeTillNextFlush => _lastFlushTime + AutoFlushInterval - DateTime.UtcNow;

		/// <summary>
		/// Save buffered data to the storage.
		/// </summary>
		...
		private void TryFlush(bool force)
		{
			...
			{
				if (!force && TimeTillNextFlush > TimeSpan.Zero)
					return;
		...
		{
			if (_storageTaskStarted || !Buffer.Enabled || Buffer.DisableStorageTimer)
				return;

			_storageTaskStarted = true;
			...
			Task.Run(async () =>
			{
				try
				{
					while (true)
					{
						var timeleft = TimeTillNextFlush;
